...
const pathOutput = path.resolve(__dirname, '../contributor.json')
...
const OWENER = process.env.OWENER
...
async function getComponents() {
  return glob('*', {
    cwd: path.resolve(root, 'examples'),
...
async function buildContributor() {
  ...
  for (const comp of components) {
    const fetchList = [
      { compName: comp, path: `examples/${comp}` },
  ...
  writeFileSync(pathOutput, JSON.stringify(contributors))
